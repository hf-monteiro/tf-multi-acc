...
provider "aws" {
  assume_role {
    role_arn = "arn:aws:iam::${data.terraform_remote_state.organization.outputs.security_acct_id}:role/Administrator"
  }
...
resource "aws_iam_account_alias" "alias" {
  account_alias = "${var.org_name}-security"
}
...
resource "aws_iam_group_policy_attachment" "security_admins_administrator" {
...
resource "aws_iam_group_policy_attachment" "security_admins_terragrunt_admin" {
  group      = aws_iam_group.security_admins.name
  policy_arn = data.terraform_remote_state.organization.outputs.terragrunt_admin_role_policy_arn
}

resource "aws_iam_group_policy_attachment" "security_admins_terragrunt_reader" {
  group      = aws_iam_group.security_admins.name
...
resource "aws_iam_group" "stage_admins" {
  name = "StageAdmins"
}

resource "aws_iam_group_policy_attachment" "stage_admins_administrator" {
  group      = aws_iam_group.stage_admins.name
  policy_arn = data.terraform_remote_state.organization.outputs.stage_admin_role_policy_arn
}

resource "aws_iam_group_policy_attachment" "stage_admins_terragrunt_admin" {
  group      = aws_iam_group.stage_admins.name
  policy_arn = data.terraform_remote_state.organization.outputs.terragrunt_admin_role_policy_arn
}

resource "aws_iam_group_policy_attachment" "stage_admins_terragrunt_reader" {
  group      = aws_iam_group.stage_admins.name
  policy_arn = data.terraform_remote_state.organization.outputs.terragrunt_reader_role_policy_arn
}
...
resource "aws_iam_group_policy_attachment" "dev_admins_administrator" {
  group      = aws_iam_group.dev_admins.name
  policy_arn = data.terraform_remote_state.organization.outputs.dev_admin_role_policy_arn
}

resource "aws_iam_group_policy_attachment" "dev_admins_terragrunt_admin" {
  group      = aws_iam_group.dev_admins.name
  policy_arn = data.terraform_remote_state.organization.outputs.terragrunt_admin_role_policy_arn
}

resource "aws_iam_group_policy_attachment" "dev_admins_terragrunt_reader" {
  group      = aws_iam_group.dev_admins.name
  policy_arn = data.terraform_remote_state.organization.outputs.terragrunt_reader_role_policy_arn
}
...
resource "aws_iam_group" "stage_developers" {
  name = "NonProdDevelopers"
}

module "assume_role_policy_stage_developers" {
  source       = "../../infrastructure-modules/assume-role-policy"
  account_name = "stage"
  account_id   = data.terraform_remote_state.organization.outputs.stage_acct_id
  role         = var.developer_role_name
}

resource "aws_iam_group_policy_attachment" "stage_developers_developer" {
  group      = aws_iam_group.stage_developers.name
  policy_arn = module.assume_role_policy_stage_developers.policy_arn
}
...
module "assume_role_policy_dev_developers" {
  source       = "../../infrastructure-modules/assume-role-policy"
  account_name = "dev"
  account_id   = data.terraform_remote_state.organization.outputs.dev_acct_id
  role         = var.developer_role_name
}

resource "aws_iam_group_policy_attachment" "dev_developers_developer" {
  group      = aws_iam_group.dev_developers.name
  policy_arn = module.assume_role_policy_dev_developers.policy_arn
}
